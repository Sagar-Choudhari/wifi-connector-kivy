...
def install_update():
    ...
    try:
        print(f"\u25CB Stopping application service...")
...
def main():
    ...
    if not file_id or not file_name:
        print(f"\u2716 No new update available.")
        return
    ...
    if not latest_version:
        print(f"\u2716 Failed to extract version from file name.")
        return

    print(f"\u25CB Installed: v{installed_version}, Available: v{latest_version}")

    if installed_version == latest_version:
        print(f"\u2714 Already updated to the latest version.")
    else:
        print(f"\u21D3 New version available: v{latest_version}. Updating...")
